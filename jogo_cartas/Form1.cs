...
namespace jogo_cartas
{
    ...
    public partial class Form1 : Form
    {
        ...
        private System.Drawing.Image CarregarImagem(string nomeImagem)
        {
            // Caminho relativo para a pasta de imagens na pasta bin\Debug ou bin\Release
            string caminhoImagem = Path.Combine(System.Windows.Forms.Application.StartupPath, "Imagens", nomeImagem);

            // Verifica se o arquivo existe e carrega a imagem
            if (File.Exists(caminhoImagem))
            {
                return System.Drawing.Image.FromFile(caminhoImagem);
            }
            else
            {
        ...
        public void BtnReiniciar_Click(object sender, EventArgs e)
        {
            ...
            num1 = 0;
            num2 = 0;
            imagem1.Image = CarregarImagem("tras_carta_vermelha.png");
            imagem2.Image = CarregarImagem("tras_carta_vermelha.png");
            imagem3.Image = CarregarImagem("tras_carta_vermelha.png");
            imagem4.Image = CarregarImagem("tras_carta_vermelha.png");
            List<PictureBox> imagens = new List<PictureBox> { imagem1, imagem2, imagem3, imagem4 };
            EmbaralharPosicoes(imagens);
            podeClicar = true;
        ...
        public void Carta_Click(object sender, EventArgs e)
        {
            ...
            if (carta != null)
            {
                ...
                if (carta == imagem1)
                {
                    carta.Image = CarregarImagem("AS_copas.jpg");
                    valor_carta_jogador = 1;
                }
                else if (carta == imagem2)
                {
                    carta.Image = CarregarImagem("rei.jpg");
                    valor_carta_jogador = 2;
                }
                else if (carta == imagem3)
                {
                    carta.Image = CarregarImagem("dama.jpg");
                    valor_carta_jogador = 3;
                }
                else if (carta == imagem4)
                {
                    carta.Image = CarregarImagem("valete.jpg");
                ...
                {
                    imagemSorteada.Image = CarregarImagem("AS_copas.jpg");
                valor_carta_computador = 1;
                }
                else if (imagemSorteada == imagem2)
                {
                    imagemSorteada.Image = CarregarImagem("rei.jpg");
                valor_carta_computador = 2;
                }
                else if (imagemSorteada == imagem3)
                {
                    imagemSorteada.Image = CarregarImagem("dama.jpg");
                valor_carta_computador = 3;
                }
                else if (imagemSorteada == imagem4)
                {
                    imagemSorteada.Image = CarregarImagem("valete.jpg");
